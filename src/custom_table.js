export default class CustomTable extends Phaser.GameObjects.Container {
    ...
    constructor(scene, x, y, data = [], fontSize = 20, fontFamily = 'Arial') {
        super(scene, x, y)
    ...
    updateTable = () => {
        ...
        const padding = this.fontSize / 4
        const lineStartX = this.x - spaceWidth / 2 + padding
        ...
        this.texts = this.tableData.map((line, y) => {
            ...
            let offsetX = 0
            const textY = this.y + y * (this.fontSize + 10) + padding
            if (y <= 1)  // 上から２本まで横の線を引く
                addLine(lineStartX, textY - padding, 0, 0, lineEndX, 0, 0)
            if (y === this.tableData.length - 1) // 一番下の線
                addLine(lineStartX, textY + (this.fontSize + 10) - padding, 0, 0, lineEndX, 0, 0)
            return line.map((d, x) => {
                const textX = this.x + offsetX
                offsetX += maxWidths[x] + spaceWidth
                // 一番左端と、縦の線
                addLine(textX, textY - 5, 0, 0, 0, this.fontSize * 1.6, 0)
                if (x === line.length - 1) {// 一番右端の線
                    addLine(textX + maxWidths[x] + spaceWidth, textY - 5, 0, 0, 0, this.fontSize * 1.6, 0)
                }
                return this.scene.add.text(textX + spaceWidth / 2 + (y === 0 ? maxWidths[x] / 2 : 0), textY, d)
                    .setColor('black')
